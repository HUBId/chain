...
- Stable patch releases can change lint or formatting behaviour. Cache toolchains in CI to avoid mid-release drift and re-validate when a new 1.83.x patch lands.
...
2. Execute the suite with `RUSTUP_TOOLCHAIN=1.83.0` explicitly to ensure CI and local behaviour match.
