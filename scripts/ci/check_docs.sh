...
RPC_GUIDE_FILE="$REPO_ROOT/docs/rpp_node_operator_guide.md"
...
if [[ ! -f "$RPC_GUIDE_FILE" ]]; then
  echo "docs/rpp_node_operator_guide.md not found at $RPC_GUIDE_FILE" >&2
  exit 1
fi
...
for reference in "${RPC_GUIDE_REFERENCES[@]}"; do
  ...
  if [[ ! -f "$reference_path" ]]; then
    echo "Expected operator guide reference file missing: $reference_path" >&2
    exit 1
  fi
  if ! grep -q "rpp_node_operator_guide" "$reference_path"; then
    echo "${reference} is missing a reference to docs/rpp_node_operator_guide.md" >&2
    exit 1
  fi
done
