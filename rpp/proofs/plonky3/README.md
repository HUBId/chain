...
deterministic proof blobs, and hash-verified setup artifacts). The workspace
can be built and tested without network access thanks to the vendored Plonky3
crates under `vendor/`.
...
* **Memory budget** – the backend currently fits within 2 GiB of RAM during
  `cargo test` runs. Future Plonky3 circuits are expected to be more demanding;
  allocate at least 8 GiB of RAM when running heavy prover workloads.
