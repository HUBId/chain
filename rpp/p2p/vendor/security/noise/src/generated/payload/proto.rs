    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(26) => msg.handshake_payload = Some(r.read_bytes(bytes)?.to_owned()),
...
impl MessageWrite for NoiseHandshakePayload {
    fn get_size(&self) -> usize {
        ...
        + self
            .handshake_payload
            .as_ref()
            .map_or(0, |payload| 1 + sizeof_len(payload.len()))
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if let Some(ref payload) = self.handshake_payload {
            w.write_with_tag(26, |w| w.write_bytes(&**payload))?;
        }
