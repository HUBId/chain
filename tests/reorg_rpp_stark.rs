...
async fn reorgs_require_rpp_stark_proofs_and_recover() -> Result<()> {
    ...
    .await
    {
        Ok(cluster) => cluster,
        Err(err) => {
            eprintln!("skipping reorg rpp stark test: {err:?}");
            return Ok(());
        }
    };
    ...
    let result = async {
        ...
        let initial_workflow = workflows
            .transaction_bundle(recipient.clone(), 1_000, 40, None)
            .context("build transaction workflow")?;
        let tx_hash = initial_workflow.tx_hash.clone();
        primary
            .orchestrator
            .submit_transaction(initial_workflow)
            .await
        ...
        let canonical_hash = format!("{:064x}", next_height + 231);
        let fork_hash = format!("{:064x}", next_height + 232);
        ...
        let mut tampered_bundle = proof_bundle.stark.clone();
        if let ChainProof::RppStark(ref mut proof) = tampered_bundle.state_proof {
            if let Some(byte) = proof.proof.first_mut() {
                *byte ^= 0xff;
            } else {
                proof.proof.push(1);
            }
            ...
            verifier
                .verify_rpp_stark_block_bundle(&tampered_bundle)
                .is_err(),
        ...
        let recovery_workflow = workflows
            .transaction_bundle(recipient, 2_000, 40, None)
            .context("build recovery transaction workflow")?;
        let recovery_hash = recovery_workflow.tx_hash.clone();
        primary
            .orchestrator
            .submit_transaction(recovery_workflow)
            .await
        ...
        for stage in stage_sequence {
            primary
                .orchestrator
                .wait_for_stage(&recovery_hash, stage, NETWORK_TIMEOUT)
                .await
                .with_context(|| format!("wait for recovery stage {stage:?}"))?;
        }

        let recovered_tip = wait_for_height(primary, baseline_height + 1).await?;
        ensure!(
            recovered_tip.header.height >= baseline_height + 1,
...
async fn wait_for_height(
    ...
    loop {
        if attempts >= MAX_BLOCK_ATTEMPTS {
            return Err(anyhow!(
                "timed out waiting for node to reach height {target_height}" 
            ));
        }
        if let Some(block) = node
            .node_handle
            .latest_block()
            .context("fetch latest block while waiting for height")?
        {
