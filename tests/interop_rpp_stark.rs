...
fn interop_verify_golden_vector_ok() -> anyhow::Result<()> {
    ...
    assert!(!public_inputs.is_empty(), "public_inputs.bin should not be empty");
    ...
    if verifier.is_ready() {
        ...
        assert!(matches!(error, rpp_verifier::RppStarkVerifierError::BackendUnavailable(_)));
    }
...
fn interop_indices_match_and_are_sorted_unique() -> anyhow::Result<()> {
    ...
    for window in indices.windows(2) {
        ...
        assert!(left < right, "indices must be strictly increasing: {left} !< {right}");
    }
...
fn interop_repeatability_is_deterministic() -> anyhow::Result<()> {
    ...
    assert_eq!(proof_first, proof_second, "proof reads should be deterministic");
    ...
    assert_eq!(digest_first, digest_second, "digest computations must be deterministic");
