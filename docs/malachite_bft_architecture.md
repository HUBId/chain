...
| --- | --- | --- | --- |
| Reputation/Tiers | Tier ≥ 3 als Mindestanforderung; Reputation beeinflusst Eintritt | `select_validators` filtert Tier < 3 und aktualisiert Gewichte über Reputation/Stake; Ledger-Audits liefern Tierdaten | ✅ Tier-Handshakes blockieren Downgrades und Admission-Control stoppt Gossip unterhalb Tier 3 (`rpp/p2p/src/peerstore.rs`, `tests/network/admission_control.rs`). |
| Timetoke | Gewichtung/Thresholds, Decay, Synchronisation | Ledger pflegt Timetoke-State (`timetoke_snapshot`/`sync_timetoke_records`), Node kreditiert Uptime-Proofs und VRF-Seed nutzt Timetoke | ✅ Snapshot-Produktion/-Konsum und Replay-Schutz laufen über `timetoke::snapshots`/`timetoke::replay`; Regressionstests decken Roundtrip & Pruning-Digests ab (`tests/consensus/timetoke_snapshots.rs`). |
...
| Proofs | Nachweis VRF/Leader/Quorum in Block-Proof | STWO- und Plonky3-Zeugen prüfen VRF-Bundles, Quorum-Wurzeln und Witness-Commitments, Tamper-Tests decken manipulierte Zertifikate ab | ✅ Circuits erzwingen VRF-/Quorum-Constraints (`rpp/proofs/stwo/aggregation/mod.rs`, `rpp/proofs/plonky3/tests.rs`, `rpp/proofs/stwo/tests/consensus_metadata.rs`); Regressionstests `consensus_certificate_tampering.rs` prüfen Zeugen-Verifikation (`tests/consensus/consensus_certificate_tampering.rs`). |
...
Derzeit sind keine weiteren Blueprint-Nacharbeiten offen; zusätzliche Anforderungen werden im Phasenplan nachgeführt.
