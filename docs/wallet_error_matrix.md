...
| `-32022 wallet error` / `wallet::errors::ProverBackendMisconfigured` | Prover backend missing/unsupported at runtime | CLI stderr, Prover tab row | Rebuild with the requested backend feature (e.g., `--features prover-stwo`) or switch `[wallet.prover].backend` to a compiled option.【F:rpp/wallet/src/rpc/error.rs†L10-L73】【F:rpp/wallet/src/wallet/mod.rs†L46-L83】 |
