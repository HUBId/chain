...
- **Rollback-Dokumentation:** Wenn der Flip zurückgerollt werden muss, nutze den Abschnitt [Rollback (Backend-Downgrade)](../zk_backends.md#rollback-backend-downgrade) für die Reihenfolge aus Konfig-Revert, Cache-Leerung und Neustarts sowie die Validierungschecks. Ergänze Incident-Logs mit `/status/node`-Snapshots, Cache-Listen und Telemetrie-Screenshots, damit Auditor:innen die erfolgreiche Rückkehr zum vorherigen Backend nachvollziehen können.【F:docs/zk_backends.md†L193-L228】
