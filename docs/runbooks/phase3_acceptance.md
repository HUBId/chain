...
- [ ] **Timetoke-SLO-Bericht archiviert.** Das wöchentliche `nightly-simnet`-Artefakt `timetoke-slo-report` (aus `cargo xtask report-timetoke-slo`) dokumentiert Erfolgsquote und Latenz der letzten sieben Tage und verweist auf die SLO-Grenzen im Timetoke-Observability-Runbook.【F:.github/workflows/nightly.yml†L1-L80】【F:docs/observability/timetoke.md†L1-L120】
...
- [ ] **Checksum-Validator nach Neustart dokumentiert.** Artefakte aus `cargo test -p rpp-chain --test snapshot_checksum_restart` zeigen Log-/Metrik-Kombinationen nach einem Neustart und belegen, dass die Prüfroutine innerhalb der `snapshot_validator.cadence_secs` weiterläuft.【F:tests/network/snapshot_checksum_restart.rs†L21-L178】【F:rpp/runtime/config.rs†L1294-L1299】
