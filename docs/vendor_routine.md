...
4. **Backend verifizieren:** Nach dem Build `cargo +nightly-2025-07-14 run -p rpp-chain -- validator --no-default-features --features prod,prover-stwo -- --dry-run` ausführen. Nur wenn der Lauf ohne den Fehlerhinweis „requires the `prover-stwo` feature“ endet, ist der STWO-Backendcode aktiv und für Deployments freigegeben.
