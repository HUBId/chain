...
3. **State-sync reconciliation** – Run `cargo test --locked --test state_sync_wallet` twice (once with `--features "wallet-integration,prover-stwo"` and once with `--features "wallet-integration,backend-rpp-stark"`) to rebuild pruned blocks, replay the pruning plan, and compare the reconstructed account map against the seeded ledger. The test logs the backend under audit and fails on any hash or balance/nonce drift so responders know which prover needs investigation.【F:tests/state_sync_wallet.rs†L58-L106】 Capture the resulting `state_sync_wallet.log` from CI artifacts or the local run and attach it to the rollout ticket. If a backend diverges, cross-check `rpp.runtime.wallet.prover.jobs`/`rpp.runtime.wallet.prover.failures` for backend-level errors and the sync-lag metrics (`rpp.runtime.wallet.sync.{wallet_height,chain_tip_height,lag.blocks}`) to confirm whether the replay stalled before accounts drifted.【F:rpp/runtime/telemetry/metrics.rs†L188-L244】【F:rpp/runtime/telemetry/metrics.rs†L244-L276】
...
## 9. Wallet RPC resilience expectations

1. **DNS and endpoint churn** – Wallet RPC clients now retry DNS/connection
   failures and transient 502/503/504 responses three times with an exponential
   backoff that starts at 50 ms before surfacing a transport error. This
   protects mobile/embedded SDK traffic when DNS records rotate or the RPC host
   is briefly unavailable; operators should expect an extra ~350 ms worst-case
   before an error is emitted under sustained failure.【F:rpp/wallet/src/rpc/client.rs†L86-L144】【F:rpp/wallet/src/rpc/client.rs†L705-L782】
2. **Rate-limit handling** – The client still honors rate-limit reset hints. A
   429 response with `x-ratelimit-reset` triggers a single sleep/retry before a
   `RateLimited` error is surfaced, so retain the header on throttling paths and
   collect the emitted logs/metrics during drills.【F:rpp/wallet/src/rpc/client.rs†L101-L132】【F:tests/sdk_mobile_embedded_smoke.rs†L1-L148】
3. **Drill both RPC backends** – When validating a release, run the mobile and
   embedded SDK smoke tests (or their equivalent staging probes) so logs include
   both token-authenticated flows. Capture the request/response traces and
   runtime metrics emitted by each flow to prove retry/backoff behavior matches
   expectations before promoting the artifacts.【F:tests/sdk_mobile_embedded_smoke.rs†L1-L148】

## 10. Regional RPC failover drills
