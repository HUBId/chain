...
## Phase-C Kontrollen (Nightly) {#phase-c-kontrollen-nightly}
...
### Highlights
- **Plonky3-Arbeiten:** Phase 2 test evidence liegt im [Produktions-Testplan](../testing/plonky3_experimental_testplan.md), im [Leistungsreport](../performance/consensus_proofs.md) und im [Runbook](../runbooks/plonky3.md); Nightly-Stressläufe sichern p95-Prover/Verifier-Latenzen und Tamper-Rejections.【F:docs/testing/plonky3_experimental_testplan.md†L1-L120】【F:docs/performance/consensus_proofs.md†L1-L200】【F:docs/runbooks/plonky3.md†L1-L200】
...
### Highlights
- **Unit-Suites:** Die Matrix aus Default-, Produktions-, Plonky3-, GUI- und Wallet-Sicherheitsläufen (`cargo xtask test-unit`) deckt deterministische Witness-/VRF-Checks ab, führt die ZSI-Renewal-Regression im erweiterten Feature-Bundle aus und veröffentlicht redaktierte Artefakte (`unit-suites-<variant>`) für jede Variante.【F:.github/workflows/ci.yml†L635-L700】【F:tests/zsi_renewal.rs†L1-L165】【F:scripts/ci/collect_test_artifacts.sh†L1-L55】
- **Integrations-Workflows:** `cargo xtask test-integration` prüft Blockproduktion, Snapshot-/Light-Client-Sync und Manipulationsschutz über alle Feature-Varianten und hängt die redaktierten Artefaktpakete `integration-workflows-<variant>` an.【F:.github/workflows/ci.yml†L768-L830】【F:scripts/ci/collect_test_artifacts.sh†L1-L55】
- **Simnet-Smoke:** `cargo xtask test-simnet` läuft als Pflichtgate `simnet-smoke` für jedes Feature-Set, deckt alle Szenarien ab, generiert die Summaries für VRF-/Quorum-Stressfälle und liefert zusätzliche Sanitized-Logs (`simnet-smoke-<variant>`).【F:.github/workflows/ci.yml†L864-L918】【F:scripts/ci/collect_test_artifacts.sh†L1-L55】
...
- **Matrix-Protokolle:** Neben den Step-Logs liegen im Actions-Tab redaktierte Artefakte (`unit-suites-<variant>`, `integration-workflows-<variant>`, `simnet-smoke-<variant>`) vor, die Laufzeiten (~12/18/22 Minuten) und Detail-Logs für jede Feature-Kombination dokumentieren.【F:.github/workflows/ci.yml†L635-L918】【F:scripts/ci/collect_test_artifacts.sh†L1-L55】
...
## Phase 3 Tracking (Kalenderwoche 20/2026) {#phase-3-tracking-kalenderwoche-20-2026}
...
| Timetoke | Timetoke-SLO-Bericht und Replay-Validator-Artefakte vollständig anhängen. | Nightly-Artefakte annotieren, Replay-Failure-Cases sammeln. | [Phase‑3 Acceptance Checklist – Timetoke](../runbooks/phase3_acceptance.md)【F:docs/runbooks/phase3_acceptance.md†L34-L61】 |
