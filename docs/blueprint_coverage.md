...
**Net result:** the Plonky3 pathway exercises the runtime plumbing with
deterministic artifacts, but the blueprint milestone for a cryptographically
sound prover/verifier integration remains open until the real backend lands.

> **TODO / Risk – Phase 1:** Track the swap from the deterministic Plonky3
> placeholder to the production prover/verifier as a gating deliverable for
> Phase 1. The experimental guard and operator warnings from ADR 0001 stay in
> force until the backend delivers genuine proofs.【F:docs/architecture/adr/0001-zk-backend-status.md†L8-L42】【F:docs/rpp_node_operator_guide.md†L33-L54】
