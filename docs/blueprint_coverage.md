...
| **Plonky3** | The [`plonky3_backend`](../prover/plonky3_backend/src/lib.rs) crate provides the production prover/verifier surface and is exercised end-to-end through the [`Plonky3Prover`](../rpp/proofs/plonky3/prover/mod.rs) and [`Plonky3Verifier`](../rpp/proofs/plonky3/verifier/mod.rs). The prover caches compiled circuits, records proof outcomes, and exports telemetry snapshots that the runtime publishes via `/status/node` and validator telemetry.【F:prover/plonky3_backend/src/lib.rs†L5-L179】【F:rpp/proofs/plonky3/prover/mod.rs†L1-L230】【F:rpp/proofs/plonky3/verifier/mod.rs†L65-L240】【F:rpp/runtime/node.rs†L161-L220】 Regression suites cover canonical wallet flows plus tampering scenarios to guard the production backend.【F:rpp/proofs/plonky3/tests.rs†L1-L220】【F:tests/plonky3_transaction_roundtrip.rs†L1-L118】【F:tests/plonky3_recursion.rs†L1-L360】 | Remaining work concentrates on operational tuning (GPU acceleration, circuit cache sizing) rather than safety rails. Operators should wire the new `backend_health` telemetry into dashboards and update alert thresholds as production data accumulates.【F:docs/rpp_node_operator_guide.md†L1-L120】 |
...
* `plonky3_backend` provides the production keygen/prove/verify surface while mapping failures into `BackendError` for upstream
  adapters.【F:prover/plonky3_backend/src/lib.rs†L5-L179】
* `Plonky3Prover` caches compiled circuits, records proof statistics, and exposes a telemetry snapshot consumed by the runtime
  when reporting backend health.【F:rpp/proofs/plonky3/prover/mod.rs†L1-L230】
* `NodeStatus.backend_health` and the validator UI surface verifier counters and the Plonky3 prover snapshot instead of ad-hoc
  warning banners, enabling automation to alert on failure spikes or empty caches.【F:rpp/runtime/node.rs†L161-L220】【F:docs/interfaces/rpc/validator_status_response.jsonschema†L1-L220】【F:validator-ui/src/types.ts†L140-L220】
* Regression fixtures cover successful flows alongside tampering of verifying keys, public inputs, and proof blobs, documenting
  the guard rails for the production backend.【F:rpp/proofs/plonky3/tests.rs†L1-L220】【F:tests/plonky3_transaction_roundtrip.rs†L1-L118】【F:tests/plonky3_recursion.rs†L1-L360】
...
**Net result:** the Plonky3 pathway now mirrors the STWO and RPP-STARK
integrations with production artefacts, telemetry, and CI coverage. Operational
focus shifts to tuning performance and alert thresholds rather than mitigating
experimental risk.
...
| Plonky3 backend enablement | Roadmap Schritt 3 (Proof system phase) | Done – Plonky3 delivers production proofs, telemetry, and CI coverage; operational guidance now focuses on monitoring `backend_health` and tuning cache/GPU resources.【F:rpp/proofs/plonky3/prover/mod.rs†L1-L230】【F:rpp/runtime/node.rs†L161-L220】【F:scripts/test.sh†L1-L220】 |
