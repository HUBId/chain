...
The `nightly-simnet` workflow runs daily at 01:30 UTC and executes `cargo xtask report-timetoke-slo`, publishing the rendered markdown as the `timetoke-slo-report` artifact. The command prefers a Prometheus datasource, falling back to log archives for local dry-runs:
