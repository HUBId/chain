...
* Offline-Checks brechen aktuell an einem `toml`-Versionskonflikt zwischen `trybuild` (`^0.9`) und den auf `0.9.6` fixierten Node-Paketen ab; die vollständigen Ausgaben liegen in `logs/cargo_metadata_offline.log` und `logs/cargo_check_offline.log`.【F:logs/cargo_metadata_offline.log†L1-L15】【F:logs/cargo_check_offline.log†L1-L15】
