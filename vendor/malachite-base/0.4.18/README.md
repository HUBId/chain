...
The workspace-wide helper [`scripts/vendor_malachite/test_segments.sh`](../../../scripts/vendor_malachite/test_segments.sh) automates segment downloads, archive merging and integrity verification for every Malachite subcrate. Run it from the repository root:

```bash
./scripts/vendor_malachite/test_segments.sh
```

The `malachite-base` run produces the following log artefacts inside [`logs/`](logs/):

- `download_segments_malachite_base_0_4_18.log` – download and manifest updates for the configured chunk plan.
- `merge_segments_malachite_base_0_4_18.log` – reconstruction of the `.crate` archive from the downloaded segments.
- `integrity_report.txt` – file-level verification summary generated by `verify_extracted_files.py`.

After the verification run the helper removes any downloaded `.part*` chunks
as well as the temporary `.crate` archive so that no binary artefacts are
checked into the repository. Set `MALACHITE_KEEP_CHUNKS=1` if you want to
keep the files locally for debugging purposes.
